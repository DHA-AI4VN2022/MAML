...
class LSTM(nn.Module):
    ...
    def __init__(self,input_size, hidden_size, num_layers, bias=True, batch_first=False, dropout=False, bidirectional=False):
        ...
        self.model = script_lstm(self.input_size, self.hidden_size, self.num_layers, self.bias,
                batch_first= False, dropout = self.dropout, bidirectional = self.bidirectional)

    def forward(self,x, state):
        h,c = state
        h,c = h.squeeze(), c.squeeze()
        if self.bidirectional:
            states = [[LSTMState(h,c) for _ in range(2)] for _ in range(self.num_layers)]
        else:
            states = [LSTMState(h,c) for _ in range(self.num_layers)]
        #input mặc định (seq_length, batch_size, input_size)
        #if batch first  (batch_size,seq_length, input_size) ==> (seq_length, batch_size, input_size)
        if self.batch_first:
            x = x.reshape((x.shape[1],x.shape[0],x.shape[2]))

        # output:
        output, states = self.model(x,states)
        output_h = []
        output_c = []
        ...
        #output: (seq_length, batch_size, input_size)
        #if batch_first: reshape to (batch_size, seq_lenght, input_size)
        if self.batch_first:
...
if __name__ == "__main__":
    test_lstm = LSTM(input_size = 16, hidden_size = 32, num_layers = 1, bias=True,
                batch_first=True, dropout= False, bidirectional=False)
    x = torch.randn(64,10,16)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    h_1 = torch.zeros(1,64, 32)
    c_1 = torch.zeros(1,64, 32)
    out, (h,c) = test_lstm(x, (h_1,c_1))
