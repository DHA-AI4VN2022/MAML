import torch
from models.lstm.model import VanillaLSTM
import os
...
class LSTMSupervisor():
    def __init__(self, args, config, target_station, train_ratio, device):
        ...
        train_dataloader, valid_dataloader, test_dataloader = get_dataloader(transformed_data, args, config, train_ratio)
        ...
        self.config = config
        # Model
        self._model = VanillaLSTM(self.config, device).to(self.device)
